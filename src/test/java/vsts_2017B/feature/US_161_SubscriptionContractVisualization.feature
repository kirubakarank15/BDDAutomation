...
Feature: Subscription Contract Visualization - Subscription Pricing Value
  ...
  Scenario Outline: To Verify Subscription Widget and its fields in DSP as CAT Login
    ...
    Examples: 
      | DealerCode | Device Type | S/N      | Services         | Cat             | Dealer          | Customer       | Base Subscription      | Additional Services |  |
  ...
  Scenario Outline: To verify Subscription pricing values for PLWEB application when inherited from Customer level
    When Set in UI "DealerCode=<DealerCode>"
    And enter or modify Subscription details as "Services=<Services>,Customer =<Customer>" inherited from customer level
  ...
  Scenario Outline: To verify Subscription pricing values for PLWEB application when inherited from Dealer level
    When Set in UI "DealerCode=<DealerCode>"
    And enter or modify Subscription details as "Services=<Services>,Dealer =<Dealer>" inherited from Dealer level
    Then Verify in UI Subscription Pricing Values are displayed for all subscriptions at Dealer level only for PLWEB application alone

    Examples: 
      | Services        | Dealer          | DealerCode |
      | ProductLink WEB | Aide (EC4-14-T) | E250       |
      |                 |                 | H160       |
      |                 |                 | N030       |
      |                 |                 | M610       |
  ...
  Scenario Outline: To verify Subscription pricing values for VisionLink application alone as Cat login
    When Set in UI "DealerCode=<DealerCode>"
    When enter or modify Subscription details as "Services=<Services>,Base Subscription =<Base Subscription>,Additional Services  =<Additional Services>"
    ...
    Examples: 
      | Services   | Base Subscription      | Additional Services | DealerCode |
  ...
  Scenario Outline: To verify Subscription pricing values for My.Cat.Com application alone as Cat login
    When Set in UI "DealerCode=<DealerCode>"
    And enter or modify Subscription details as "Services=<Services>,Base Subscription =<Base Subscription>"
    ...
    Examples: 
      | Services   | Base Subscription | DealerCode |
  ...
  Scenario Outline: To Verify Subscription pricing values are not displayed for Onboard analytics Add on Subscription for PLWEB as Cat login
    ...
    Examples: 
      | Services        | Customer     | Dealer       | Cat              | Additional Service | DealerCode |
