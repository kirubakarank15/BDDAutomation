...
public class MyWorklist {
	...
	@FindBy(xpath = "//input[contains(@ng-model,'makeVal')]")
	private WebElement TxtMakeCode;

	@FindBys({ @FindBy(xpath = "//li[contains(@id,'typeahead')]//a") })
	private List<WebElement> typeaheadMakeCode;
	@FindBy(xpath = "//button[contains(@class,'btn-primary') and  contains(text(),'Search')]")
	private WebElement btnSearchMakeCodePopUp;
	...
	public MyWorklist searchAssetDetails(String fieldName, String testData) throws CustomisedException, Exception {
		...
		try {
			List<WebElement> nullCheck = TestFunctionsFactory.driver
					.findElements(By.xpath("//span[contains(text(),'Back')]"));

			if (nullCheck.size() != 0) {
				TestFunctionsFactory.javaScriptClick(linkback);
				List<WebElement> obj = TestFunctionsFactory.driver
						.findElements(By.xpath("//button[contains(text(),'Cancel')]"));
				if (obj.size() != 0) {
			...
			case "MAKECODE":
				...
				TestFunctionsFactory.webEditText(TxtMakeCode, testData);
				TestFunctionsFactory.webWait(20, typeaheadMakeCode.get(0));
				
				TestFunctionsFactory.selectFromTypeAhead(typeaheadMakeCode, testData);
			
				TestFunctionsFactory.javaScriptClick(btnSearchMakeCodePopUp);
				break;
	...
	public MyWorklist makeCodeSerach(String makeCode) throws CustomisedException, Exception {
